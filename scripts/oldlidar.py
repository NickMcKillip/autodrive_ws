...
class DataLoader():
    ...
    def __init__(self):
        ...
        self.publisher = rospy.Publisher('lidar_viz', MarkerArray, queue_size=1)
    ...
    def set_point_cloud(self, msg):
        ...
        #np.save("input.npy",self.point_cloud)
    ...
    def get_frustum(self):
        ...
        img_points[:, 0] = img_points_hom[:, 0]/img_points_hom[:, 2] * -1
        img_points[:, 1] = img_points_hom[:, 1]/img_points_hom[:, 2] * -1
        ...
        # expand the 2D bbox slightly:6
        u_min_expanded = u_min - (u_max-u_min)
        u_max_expanded = u_max + (u_max-u_min)
        v_min_expanded = v_min - (v_max-v_min)
        v_max_expanded = v_max + (v_max-v_min)
        ...
        try:
            ...
            print("No points in Frustum Point Cloud")
        ...
        print("IMG center", center_img_point_hom)
        #print(u_center)
        #print(v_center)
        ...
        print("Point hom", point_hom)

        # hom --> normal coords:
        point = point_hom[:3] #np.array(([point_hom[0]/point_hom[3], point_hom[1]/point_hom[3], point_hom[2]/point_hom[3]]))
        print("Point",point)

        markerArray2 = MarkerArray()

        marker = Marker()
        marker.header.frame_id = "lidar_center"
        ...
        #self.publisher.publish(markerArray2)
        ...
        print("Angle:", self.frustum_angle)
        #self.frustum_angle = 0
        ...
        self.frustum_point_clouds = self.frustum_point_clouds.transpose(1,0)
        self.pred()
    ...
    def preds_to_3dbb(self):
       ...
       # print(row_mask.shape)
        ...
        #print(bbox_dict)
        print("Pred center", pred_center_BboxNet)
        markerArray2 = MarkerArray()
        marker = Marker()
        marker.header.frame_id = "lidar_center"
        marker.type = marker.SPHERE
        ...
        marker.pose.position.z = -pred_center_BboxNet[1]/2
        marker.pose.position.x = pred_center_BboxNet[2]/2
        marker.pose.position.y = -pred_center_BboxNet[0]/2

        #marker.pose.position.z = -pred_center_BboxNet[1]
        #marker.pose.position.x = pred_center_BboxNet[0]
        #marker.pose.position.y = pred_center_BboxNet[2]
        ...
        obs = Obstacle()
        obs.x1 = (pred_center_BboxNet[2] - pred_l/2)/2
        obs.x2 = (pred_center_BboxNet[2] + pred_l/2)/2
        obs.y1 = (-pred_center_BboxNet[0] - pred_w/2)/2
        obs.y2 = (-pred_center_BboxNet[0] + pred_w_2)/2
